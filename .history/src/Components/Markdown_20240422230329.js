  ...
  return (
    <div className="container">
      <div className="notes-container">
        <h2>Notes</h2>
        <button className="button" onClick={handleNoteAdd}>+</button>
        <ul>
          {notes.map((note) => (
            <li key={note.id}>
              Note {note.id}
              <button className="button" onClick={() => handleNoteClick(note.id)}><FontAwesomeIcon icon={faEdit} /></button>
      ...
      <div className="markdown-container">
        ...
        <div>
          <button className="button" onClick={() => setShowEditor(true)}><FontAwesomeIcon icon={faPencilAlt} /></button>
          <button className="button" onClick={() => setShowEditor(false)}><FontAwesomeIcon icon={faEye} /></button>
        </div>
        {showEditor ? (
          <textarea
            ref={textareaRef}
